...
|---|---|
| Tên use case | UC03 |
| Tác nhân chính | User (Sinh viên / Khách truy cập / Giảng viên) |
| Mục đích (mô tả) | Cho phép người dùng tìm kiếm tài liệu nhanh thông qua từ khóa kết hợp với các bộ lọc chi tiết (ngành học, môn học, loại tài liệu, đánh giá, thời gian). Hệ thống hỗ trợ tìm kiếm kết hợp: ưu tiên tìm kiếm chính xác theo từ khóa trước, nếu không có kết quả phù hợp sẽ tự động chuyển sang tìm kiếm theo ngữ nghĩa bằng trí tuệ nhân tạo. |
| Mức độ ưu tiên (Priority) | Bắt buộc (Quan trọng) |
| Điều kiện kích hoạt (Trigger) | User truy cập vào trang Tìm kiếm nâng cao hoặc thực hiện bấm "Áp dụng" / "Tìm kiếm". |
| Điều kiện tiên quyết (Pre-condition) | - Không yêu cầu đăng nhập. |
| Điều kiện thành công (Post-condition) | - Hệ thống hiển thị danh sách tài liệu khớp với các tiêu chí tìm kiếm kèm mức độ phù hợp tương ứng cho người dùng. |
| Điều kiện thất bại | - Hệ thống gặp lỗi kết nối hoặc không thể truy xuất kho dữ liệu.<br>- Hiển thị thông báo lỗi cho người dùng. |
| Luồng sự kiện chính (Basic Flow) | 1. User truy cập màn hình Tìm kiếm nâng cao.<br>2. Hệ thống tải danh sách các ngành học, môn học và hiển thị lên các bộ lọc chọn lựa.<br>3. User nhập từ khóa và chọn các tiêu chí lọc mong muốn (Ngành học, Môn học, Loại tài liệu, Đánh giá tối thiểu, Thời gian cập nhật).<br>4. User bấm nút "Tìm kiếm" hoặc "Áp dụng".<br>5. Hệ thống tiếp nhận từ khóa và các tiêu chí lọc.<br>6. Hệ thống thực hiện tìm kiếm và phân loại dữ liệu:<br>&nbsp;&nbsp;&nbsp;&nbsp;**[Trường hợp A] Tìm kiếm theo từ khóa thông thường**:<br>&nbsp;&nbsp;&nbsp;&nbsp;- Hệ thống tìm kiếm các tài liệu có thông tin (tiêu đề, mô tả...) khớp với từ khóa và thỏa mãn các bộ lọc đang chọn.<br>&nbsp;&nbsp;&nbsp;&nbsp;- Nếu tìm thấy kết quả hoặc người dùng không nhập từ khóa: Hệ thống chuyển trực tiếp sang bước 7.<br>&nbsp;&nbsp;&nbsp;&nbsp;**[Trường hợp B] Tìm kiếm theo ngữ nghĩa bằng AI** (Chạy tự động khi Trường hợp A không có kết quả):<br>&nbsp;&nbsp;&nbsp;&nbsp;- Hệ thống sử dụng mô hình trí tuệ nhân tạo để phân tích ý nghĩa của từ khóa.<br>&nbsp;&nbsp;&nbsp;&nbsp;- Tìm kiếm và đối chiếu các tài liệu trong kho dữ liệu có nội dung liên quan mật thiết nhất với ý nghĩa từ khóa và thỏa mãn các bộ lọc đang chọn.<br>7. Hệ thống tổng hợp danh sách tài liệu tìm được.<br>8. Hệ thống hiển thị kết quả lên giao diện dưới dạng danh sách các thẻ tài liệu. Đối với tài liệu tìm kiếm theo ngữ nghĩa, hệ thống hiển thị thêm nhãn hiệu **"Độ phù hợp x%"** và tự động sắp xếp theo thứ tự **"Liên quan nhất"**. |
| Luồng sự kiện thay thế (Alternative Flow) | 3a. User bấm nút "Xóa": Hệ thống thiết lập lại toàn bộ các tiêu chí bộ lọc về trạng thái mặc định ("Tất cả").<br>8a. Nếu không tìm thấy kết quả phù hợp: Hệ thống hiển thị thông báo "Chưa có kết quả tìm kiếm" và gợi ý thay đổi từ khóa hoặc bộ lọc. |
| Luồng sự kiện ngoại lệ (Exception Flow) | 2a. Không tải được danh sách Ngành/Môn học: Bộ lọc dropdown sẽ để trống, người dùng vẫn có thể tìm kiếm bình thường bằng từ khóa.<br>6a. Hệ thống gặp lỗi xử lý dữ liệu hoặc mất kết nối dịch vụ tìm kiếm: Hệ thống hiển thị thông báo "Có lỗi khi tìm kiếm. Vui lòng thử lại sau." |
| Cơ sở dữ liệu | Hệ thống truy xuất dữ liệu từ:<br>- Kho lưu trữ tài liệu (thông tin tài liệu, tệp tin, loại tài liệu, lượt tải, lượt xem, đánh giá).<br>- Danh mục quản lý ngành học và môn học.<br>- Hệ thống lưu trữ và truy vấn vector ngữ nghĩa tài liệu. |
